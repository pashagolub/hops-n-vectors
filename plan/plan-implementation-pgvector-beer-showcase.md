...
│   └── 02_timetable_job.sql            # rebuild_embeddings chain (idempotent)
...
## Phase 5 — Scheduled Re-embedding (pg_timetable)

Goal: incremental rebuild job reacting to data edits.

| Task | Description | Files | Spec refs | Depends on |
|---|---|---|---|---|
| **5.1** | Add a re-embed entry point (e.g., `python -m hopsnvectors.embedder --once`) reusing 4.1 logic; safe for concurrent invocation | `app/hopsnvectors/embedder.py` | REQ-010, PAT-002 | 4.1 |
| **5.2** | Finalize `db/init/02_timetable_job.sql`: idempotent `timetable.add_job` (or chain definition) named `rebuild_embeddings`, default `*/15 * * * *`, overridable via `${REBUILD_SCHEDULE}`; the chain triggers the re-embed logic (program task invoking embedder in the app container, or an SQL task marking rows + a compose sidecar — choose the simplest observable mechanism and document it) | `db/init/02_timetable_job.sql`, `docker-compose.yml` | REQ-010, §4.4, AC-010 | 5.1, 2.3 |

**Phase gate:** `SELECT * FROM timetable.chain;` shows `rebuild_embeddings` (AC-010); after `UPDATE beers SET info = info || ' extra hoppy' WHERE id = 1;` the next job run re-embeds only that row — verify via `embedded_at` (AC-005); `timetable.execution_log` shows ≥ 1 successful run.
...
| **6.5** | Wire `tui` service into compose (profile or `docker compose run --rm tui`), `depends_on: embedder: condition: service_completed_successfully`, model-cache mounted | `docker-compose.yml` | §4.1 | 6.2, 4.3 |

**Phase gate:** `docker compose run --rm tui --query lemon --json` returns 5 results as valid JSON in < 2 s warm (AC-002, PER-003); interactive commands behave per §4.5.
...
| **9.2** | E2E script: `docker compose up -d`, wait for ready, assert AC-001/AC-002/AC-003 via `tui --query lemon --json` and psql counts; record startup + embedding time, fail if over PER-002 budget | `.github/workflows/ci.yml` or `scripts/e2e.sh` | §6 E2E, PER-002 | Phases 4, 6 |
...
| pg_timetable → embedder invocation mechanism unclear | Phase 5 stalls | Task 5.2 explicitly allows choosing simplest observable mechanism; decide and document before implementing |
